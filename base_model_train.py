...
from base_model import WrappedModel, UNet_module
...
class train:
    ...
    def __init__(self, image, mask, ignore=None, sky=None, aug_sky=(0, 0), aug_img=(1, 1), noise=False, saturation=1e5,
        ...
        if gpu:
            ...
            self.network = nn.DataParallel(UNet_module(1,1,hidden, norm='group', norm_setting=[8,0,True], conv_type='unet', down_type='maxpool', up_type='transconv', att=False, deeper=False))
            self.network.type(self.dtype)
        else:
            self.dtype = torch.FloatTensor
            self.dint = torch.ByteTensor
            self.network = WrappedModel(UNet_module(1,1,hidden, norm='group', norm_setting=[8,0,True], conv_type='unet', down_type='maxpool', up_type='transconv', att=False, deeper=False))
            self.network.type(self.dtype)
